...
import '../services/mexc_futures_config.dart';
...
class TradingProvider with ChangeNotifier {
  ...
  // Force leverage = 1 and fixed order size 1 USDT as requested
  final int _defaultLeverage = MexcFuturesConfig.defaultLeverage; // = 1
  final double _fixedOrderSize = MexcFuturesConfig.fixedOrderSizeUsdt; // = 1.0
  ...
  Future<bool> placeTrade({
    ...
    try {
      ...
      // Set leverage = 1 (no extra leverage) before trading
      ...
      // Use fixed 1 USDT order size as requested
      final double tradeAmount = amount <= 0 ? _fixedOrderSize : amount;

      final Map<String, dynamic> orderPayload = {
        'symbol': symbol,
        'price': orderPrice,
        'vol': tradeAmount.toInt(),
        'leverage': _defaultLeverage,
        'side': orderSide,
        'type': orderType,
        'openType': 1,
  ...
  Future<void> _botCycle() async {
    ...
    try {
      await syncBalance();
      if (_balance < 1) {
        _error = 'رصيد غير كافٍ للبوت (الحد الأدنى 1 USDT)';
        _isTrading = false;
        notifyListeners();
        return;
      }
      ...
      if (signal == 'BUY' || signal == 'SELL') {
        // Fixed 1 USDT per trade, no leverage
        final tradeAmount = _fixedOrderSize;
