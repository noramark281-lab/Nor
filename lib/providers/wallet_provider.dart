...
class WalletProvider with ChangeNotifier {
  ...
  // ── Order Management ────────────────────────────────────────────
  /// إلغاء أمر محدد - يدعم الاستدعاء بـ (symbol, orderId) أو (orderId) فقط
  Future<bool> cancelOrder(dynamic symbolOrOrderId, [String? maybeOrderId]) async {
    _loading = true;
    notifyListeners();
    ...
    try {
      ...
      if (maybeOrderId != null) {
        symbol = symbolOrOrderId.toString();
        orderId = maybeOrderId;
      } else {
        orderId = symbolOrOrderId.toString();
        // ابحث عن الرمز في الأوامر المفتوحة
        final matchingOrder = _openOrders.firstWhere(
          (o) => o['orderId']?.toString() == orderId,
          orElse: () => <String, dynamic>{},
        );
        symbol = matchingOrder['symbol']?.toString() ?? '';
      }
  ...
  // ── Helpers ─────────────────────────────────────────────────────
  String formatAssetValue(dynamic value) {
    if (value == null) return '0.00';
    final numVal = value is num ? value.toDouble() : double.tryParse(value.toString()) ?? 0.0;
    if (numVal >= 1000000) return '${(numVal / 1000000).toStringAsFixed(2)}M';
    if (numVal >= 1000) return '${(numVal / 1000).toStringAsFixed(2)}K';
    return numVal.toStringAsFixed(numVal < 1 && numVal > 0 ? 6 : 2);
  }
