...
import { auth, firebaseSync } from './firebase'
...
export const db = {
  async getSettings() {
    ...
    if (user) {
      try {
        const cloudSettings = await firebaseSync.getSettings(user.uid)
        if (cloudSettings) {
          localStorage.setItem('nor_settings', JSON.stringify(cloudSettings))
          return cloudSettings
        }
      } catch (e) {
        console.warn('Firebase settings fetch fallback to local:', e)
      }
    }
  ...
  async saveSettings(settings: any) {
    ...
    if (user) {
      try {
        await firebaseSync.saveSettings(user.uid, updated)
      } catch (e) {
        console.warn('Firebase saveSettings sync warning:', e)
      }
    }
    return updated
  },
  ...
  async getTrades(limit = 100) {
    ...
    if (user) {
      try {
        const cloudTrades = await firebaseSync.getTrades(user.uid, limit)
        if (cloudTrades && cloudTrades.length > 0) {
          localStorage.setItem('nor_trades', JSON.stringify(cloudTrades))
          return cloudTrades
        }
      } catch (e) {
        console.warn('Firebase trades fetch fallback to local:', e)
      }
    }
  ...
  async addTrade(trade: any) {
    ...
    if (user) {
      try {
        await firebaseSync.addTrade(user.uid, trade)
      } catch (e) {
        console.warn('Firebase addTrade sync warning:', e)
      }
    }
    return trade
  },
  ...
  async getBotTrades(limit = 100) {
    ...
    if (user) {
      try {
        const cloudBotTrades = await firebaseSync.getBotTrades(user.uid, limit)
        if (cloudBotTrades && cloudBotTrades.length > 0) {
          localStorage.setItem('nor_bot_trades', JSON.stringify(cloudBotTrades))
          return cloudBotTrades
        }
      } catch (e) {
        console.warn('Firebase bot trades fetch fallback to local:', e)
      }
    }
  ...
  async addBotTrade(trade: any) {
    ...
    if (user) {
      try {
        await firebaseSync.addBotTrade(user.uid, trade)
      } catch (e) {
        console.warn('Firebase addBotTrade sync warning:', e)
      }
    }
    return trade
  },
  ...
  async getActivePositions(): Promise<ActivePosition[]> {
    ...
    if (user) {
      try {
        const cloudPositions = await firebaseSync.getPositions(user.uid)
        if (cloudPositions && cloudPositions.length > 0) {
          localStorage.setItem('nor_active_positions', JSON.stringify(cloudPositions))
          return cloudPositions as ActivePosition[]
        }
      } catch (e) {
        console.warn('Firebase getPositions fallback to local:', e)
      }
    }
  ...
  async addActivePosition(position: ActivePosition) {
    ...
    if (user) {
      try {
        await firebaseSync.addPosition(user.uid, position)
      } catch (e) {
        console.warn('Firebase addPosition sync warning:', e)
      }
    }
  },
  ...
  async removeActivePosition(id: string) {
    ...
    if (user) {
      try {
        await firebaseSync.removePosition(user.uid, id)
      } catch (e) {
        console.warn('Firebase removePosition sync warning:', e)
      }
    }
  },
  ...
  async getDustLogs() {
    ...
    if (user) {
      try {
        const cloudLogs = await firebaseSync.getDustLogs(user.uid)
        if (cloudLogs && cloudLogs.length > 0) {
          localStorage.setItem('nor_dust_logs', JSON.stringify(cloudLogs))
          return cloudLogs
        }
      } catch (e) {
        console.warn('Firebase getDustLogs fallback to local:', e)
      }
    }
  ...
  async addDustLog(log: any) {
    ...
    if (user) {
      try {
        await firebaseSync.addDustLog(user.uid, log)
      } catch (e) {
        console.warn('Firebase addDustLog sync warning:', e)
      }
    }
  },
