...
class MexcApiManager {
  ...
  Future<void> setCredentials({required String apiKey, required String secretKey}) async {
    ...
    if (_apiKey!.isEmpty || _secretKey!.isEmpty) throw Exception('يجب إدخال API Key و Secret Key.');
    final prefs = await SharedPreferences.getInstance();
  ...
  Map<String, String> getSpotHeaders() {
    if (!_isInitialized || _apiKey == null || _apiKey!.isEmpty) throw Exception('MEXC Spot API credentials are not configured.');
    return {'Content-Type': 'application/json', 'X-MEXC-APIKEY': _apiKey!, 'Accept': 'application/json'};
  }
  ...
  Future<bool> testConnection() async {
    return _isInitialized && (_apiKey?.isNotEmpty ?? false) && (_secretKey?.isNotEmpty ?? false);
  }
}
