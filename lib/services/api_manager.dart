...
/// مدير API آمن مع توقيع HMAC-SHA256
class MexcApiManager {
  ...
  Future<http.Response> signedGet(String endpoint, {Map<String, String>? params}) async {
    ...
    final url = '${AppConstants.mexcBaseUrl}$endpoint?$queryString&signature=$signature';
  ...
  Future<http.Response> signedPost(String endpoint, Map<String, dynamic> body) async {
    ...
    return await http.post(
      Uri.parse('${AppConstants.mexcBaseUrl}$endpoint?signature=$signature'),
      headers: {
        'X-MEXC-APIKEY': _apiKey!,
        'Content-Type': 'application/json',
      },
  ...
  Future<http.Response> publicGet(String endpoint, {Map<String, String>? params}) async {
    var url = '${AppConstants.mexcBaseUrl}$endpoint';
